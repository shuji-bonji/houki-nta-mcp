...
6. **`limit` の範囲外の値を黙って丸める。** `limit` が 1 未満なら 1、50 を超えれば 50 として探す（エラーにも `hint` にもしない）。意図として認めるか、`INVALID_ARGUMENT` にするか。テストも無い。
7. **空のキーワード。** `keyword` が空文字や空白だけ、または 1 文字の語だけのときは検索条件が無くなり、DB にタックスアンサーがあれば `results: []` と「該当なし」の `hint` を返す（合う文書が無いのではなく、探していない）。`INVALID_ARGUMENT` にするか、`hint` で区別するか。テストも無い。
8. **`nta_get_tax_answer` へ進む案内が無い。** 結果の要素に番号（`docId`）はあるが、`next_actions` で `nta_get_tax_answer` を案内しない。他の取得系ツールの応答と揃えるか。
