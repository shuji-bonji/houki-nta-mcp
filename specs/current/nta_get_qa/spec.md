...
- 機能 ID: NTA
- 版: current
- 承認日: （未承認。初版起こしの草案。人が承認したときに日付を書く）
...
1. **存在しない事例を指定したときのエラー。** 国税庁サイトが 404 を返すと、エラー `SOURCE_API_ERROR`（`retryable: true`、`next_actions` は時間をおいて再試行する案内）になる。事例番号の誤りは再試行しても直らない。`nta_get_bunshokaitou` などの `DOC_NOT_FOUND` のように、番号の誤りと取得の失敗を分けるか。このエラー自体もこのツールのテストが無い。ID を振るのは受入テストを書いてから。
2. **`category` と `id` の形を確かめない。** 空かどうかだけを見て、数字であることは確かめない（`"abc"` や `/` を含む値もそのまま URL に入る）。取得先のホストは `www.nta.go.jp` に固定されているが、形の検査を足すか。
