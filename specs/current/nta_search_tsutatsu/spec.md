...
1. **`limit` の範囲外の値の扱い。** 0 以下は 1 に、50 を超える値は 50 に丸めて検索する（エラーにしない）。inputSchema の説明には「最大: 50」とあるが、超えたときにエラーにするか丸めるかは書いていない。丸めることのこのツールの応答としてのテストが無い。ID を振るのは受入テストを書いてから。
...
8. **`TSUTATSU_NOT_FOUND` の案内するフラグ。** SPEC-NTA-SEARCH-TSUTATSU-003 の `hint` と `next_actions` は `--bulk-download`（通達 1 つ。既定は消費税法基本通達）を案内するが、ツールの説明文と `freshness.warning` は `--bulk-download-all`（4 種）を案内する。どちらに揃えるか。
9. **`keyword` が空のときの `INVALID_ARGUMENT` の形。** SPEC-NTA-SEARCH-TSUTATSU-002 の応答には `tool` は付くが、`hint`・`next_actions`・`detail` は付かない（SPEC-NTA-SEARCH-TSUTATSU-001 の inputSchema 違反のときは付く）。揃えるか。
10. **0 件のときの `count`。** ヒットしたときは `count` が付くが、0 件のときは付かない（`hits: []` と `message` で示す）。文書系の検索（`nta_search_qa` など）と応答の形（`hits` / `results`、`message` / `hint`）も違う。意図として認めるか、揃えるか。
