...
- 承認日は、人がマージの前にそのブランチで書きます。仕様 PR は proposal.md に「- 承認日: YYYY-MM-DD（PR #N）」、初版起こしと取り込みは `specs/current/<dir>/spec.md` に「- 承認日: YYYY-MM-DD」。日付は JST です。マージの後に書くと、承認日を書くためだけの PR が要るため、マージの前に書きます。空欄なら `pr-scope` が止めます。
