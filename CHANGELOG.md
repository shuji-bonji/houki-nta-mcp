...
- **DB の経路で `ARTICLE_NOT_FOUND` を返す条件**（SPEC-NTA-GET-TSUTATSU-005）: 条項が DB に無いとき、その通達を bulk download で全章取り込んであるときだけ `available_clauses`（最大 50 件）を返し、国税庁サイトへは取りに行かない。国税庁サイトから取って書き戻した節しか無い通達は、国税庁サイトへ進む。「全章を取り込んだ」は `tsutatsu.bulk_completed_at` で判定する（bulk download が全章を取り終えたときに書く。`onlyChapter` の実行と書き戻しでは書かない）
- **国税庁サイトから取るページの決め方**（006）: 消費税法基本通達は今までどおり `{章}/{節}.htm` を先に取り、そこに無ければ目次で選び直す（節が款に分かれていれば款のページ）。法人税基本通達は目次の章（`第12章の2`）と節（`第3節の2`、款なら属する節）の題から、所得税基本通達は「法第N条…関係」「法第N条から第M条まで…共通関係」と「〔…〕」の項目が属する見出しの題から、相続税法基本通達は「第N条《…》関係」「…及び…共通関係」の見出しの題から候補ページを選ぶ。候補が複数なら目次の順に取り、見つかったところで止める。取得して解析できたページは、見つからなかったページも DB に書き戻す
- **番号の形の検査**（008）: `clause` を通達ごとの番号の形（消基通・法基通は章-節-条、所基通・相基通は条-項と共通の通達の形）で読み、当たらなければ `INVALID_ARGUMENT` にして `hint` にその通達の形と例を書く。全角の数字・ハイフンは、国税庁サイトから取るときも半角に揃えてから読む（仕様の未決 1 の解消）。番号の形には当たるが、目次（取り直した後も）から候補ページを 1 つも決められないときも `INVALID_ARGUMENT`
- **`SOURCE_API_ERROR`**（009）: 目次・候補ページの取得が通信の失敗やサイトのエラーで失敗したときだけに出す。候補ページが存在しない（404、国税庁サイトの 404 ページへの転送）ことでは出さず、次の候補へ進む
- **候補ページのどれにも条項が無いとき**（010）: `ARTICLE_NOT_FOUND` に、取得したページにある条項番号（`available_clauses`）と、取得したページの URL（`searched_urls`、新しい項目）を入れる。`hint` に番号の形の確認・`nta_search_tsutatsu` での検索・`--bulk-download` を書き、`next_actions` に同じ 2 つを入れる
- `nta_get_tsutatsu` の `description` と `clause` の説明文（`tools/list`）を、上の振る舞いに合わせて書き替えた

### Added

- **目次の保存と取り直し**（014）: 候補ページを決めるために取得した目次の解析結果を、新しい表 `tsutatsu_toc`（目次の URL を鍵に、解析結果の JSON・取得日時・`Last-Modified`・`ETag`）に保存し、次の呼び出しでは国税庁サイトから取り直さずに使う。使い回す期間は設けない。候補を決められない・どの候補にも条項が無い・どの候補も存在しないときにだけ、1 回の呼び出しにつき 1 回、前回の `Last-Modified` / `ETag` を付けて目次を取り直す。304 なら取り直さずにエラーを返す。その呼び出しで取得したばかりの目次は取り直さない
- **1 回の呼び出しで取るページの上限**（015）: 候補ページは 10 まで（目次ページは数えない）、ページとページのあいだは 0.3 秒あける（`TSUTATSU_LIVE_FETCH`）。上限に達して見つからなければ `ARTICLE_NOT_FOUND` の `hint` に上限に達したことと、見ていない候補ページの数を書く。上限に達したときは目次を取り直さない
- 目次の解析器に 2 つ足した（bulk download の章・節の番号は変わらない）: 法人税基本通達の款の項目に、属する節の題（`parentTitle`）。所得税・相続税法基本通達の項目に、そのページに結び付く条を示す題の一覧（`articleTitles`）。同じページを指す 2 つ目以降の項目（`04/01.htm#a-02` の「法第24条…関係」、相基通 `01/01.htm` の「第2条…及び第2条の2…共通関係」）の題も持つ
- `getTsutatsu` のオプションに、国税庁サイトから取るときの上限・間隔・再試行（`maxPages` / `pageIntervalMs` / `maxRetries` / `retryBaseMs`）を足した（テストで短くする用）
- 国税庁サイトから取る処理は `src/services/tsutatsu-live.ts` にまとめた

### DB（schema v9 → v10）

- `tsutatsu` に `bulk_completed_at` を足し、`tsutatsu_toc` 表を作る。既存の DB では、bulk download が書いた節（`last_modified` か `etag` が入っている）を持つ通達を「bulk download 済み」とみなし、その節の `fetched_at` の最大値で埋める。書き戻した節しか無い通達は NULL のまま。国税庁サイトへのアクセスは発生しない

### Tests

- 受入テスト（このブランチの `test:` コミット 2 件、005・006・008・009・010・014・015）が通る。期待値は変えていない
- `src/services/tsutatsu-live.test.ts`（番号の読み方・目次の題からの条の読み方・候補ページの選び方・目次の保存）、bulk download の `bulk_completed_at` の 2 件、schema v9 → v10 の 5 件を足した
- schema の既存テスト 3 件（v4 → v5、v7 → v8、v8 → v9 の「schema_version が最新になる」）の `expect(SCHEMA_VERSION).toBe(9)` を `toBe(10)` にした。v10 を足したことで最新の番号が変わったため

## [0.20.3] - 2026-09-24

**patch リリース** — `nta_get_tsutatsu` の `clause` の説明文を、通達ごとの番号の形に合わせた。振る舞いは変わらない。
...
- **`--quickstart`**: 通達 1 本（既定: 消費税法基本通達。`--tsutatsu=<正式名>` で変更可）だけを投入する。約 3〜5 分。終わると、その通達に対して `nta_search_tsutatsu` と `nta_get_tsutatsu` が使える。処理は `--bulk-download` と同じで、違いは実行前に何を入れるかを出し、完了後に次の一手（別の通達を足す / 4 種 / 全部入り）を出すこと
- **`--bulk-download-everything` の開始時に、種別ごとの件数と所要時間の目安を表示する**（`formatBulkEstimates`）。合計 約 100 分と、税目で絞る方法、`--quickstart` への案内を含む。README「初回セットアップ」の表と同じ値

### Changed

- **`--help` の並びを「まず試す → 種別を足す → 全部入り → 保守」に変えた**。これまでは `--bulk-download-everything`（約 100 分）が先頭で「推奨」と書いてあり、初めて入れた人がそこから始めていた。DB が無くても `nta_get_*` は国税庁サイトから直接取れること（検索だけは DB が要る）も `--help` に書いた
- **README の「初回セットアップ」の先頭に「まず数分で試す」を置いた**。あわせて、2 箇所に残っていた「約 50 分」を「約 100 分」に直した（表の合計と `--help` は以前から約 100 分だった）

### 背景

houki-hub#22（発見性 — 1 種類の仕事を 1 回の導入で終わらせる）の (b) 導入の時間。plugin の `dependencies`（houki-research v0.7.0）で導入の手数は 1 回になったが、入れた直後に約 100 分の取り込みが始まる状態は変わっていなかった。
...
- **`related_laws` / `related_tsutatsu`**: `nta_get_qa`（`format: "json"`）の応答に、【関係法令通達】欄を分けた参照を載せる（`src/services/related-law-parser.ts` の `parseRelatedReferences()`）。法令は `law_name` / `article`（houki-egov-mcp の `get_law` と同じ「57の2」の形）/ `paragraph` / `item` / `appendix`（別表）、通達は `name` / `clause`。どちらも元の要素を `raw` に残す
  - 「所得税法第27条、第34条」の「第34条」は直前の法令の条、「、第9項」は直前の条の項、「、第12号」は直前の条・項の号、「、3-3」「、185」は直前の通達の番号として読む
  - 「第12号の8」のような枝番号の号は `item` に入れない（数値で表せないため。`raw` で確かめる）
  - 告示・説明文・「［参考］」など法令と通達として読めない要素は入れない。読めない要素の後ろの「第52条」のような続きも、法令名が分からないので読まない
  - 箇条書きの番号（「1 相続税法第23条」）は法令名に含めない。区切りが抜けた「消費税法施行令第45条第3項消費税法基本通達…」は法令と通達に分ける
- **成功時の `next_actions`**: 条まで読めた法令は houki-egov-mcp の `get_law`（`law_name` / `article` / `paragraph` / `item` をそのまま渡せる）、基本通達 4 種で番号まで読めたものは `nta_get_tsutatsu`（番号の後ろの「(4)」のような細目は除く）へ案内する。条約（通称で e-Gov の法令名と一致しない）と改正前の法令（「旧所得税法」「改正前厚生年金保険法」。`get_law` は現行の条文を返す）は案内しない。同じ案内は 1 回だけ
- **`qa.notice` / `qa.basisDate`**: ページ下部の「注記」の本文（作成時点と、一般的な回答であり具体的な取引に当てはめると異なる課税関係が生じうる旨の断り書き）と、「令和7年8月1日現在の法令・通達等に基づいて作成」から取った作成基準日（例: `2025-08-01`）
- Markdown 応答（既定）に「## 注記（国税庁）」の節を追加
- `nta_get_qa` のツール説明（`tools/list` の `description`）に上のフィールドを追記
- テスト 20 件を追加（`related-law-parser.test.ts` 16 件、`qa-parser.test.ts` 3 件、`handlers.test.ts` 1 件。合計 **575 tests**）

### Fixed

- **「注記」が `relatedLaws` に混ざっていた** — 注記（`<p class="red"><strong>注記<br>…`）の後ろには見出し（`<h2>`）が無いので、最後の節の段落として拾われていた。ローカル DB の 1,834 件すべてで、`relatedLaws` の最後の要素が注記だった。【関係法令通達】欄の無いページ（3 件）では `answer` に混ざっていた。どの節からも外して `qa.notice` に分けた

### 分解できた割合

2026-09-11 に、ローカル DB の質疑応答事例 1,841 件のうち【関係法令通達】欄のある 1,834 件で測った。

| | 件数 |
| --- | --- |
| 欄を「、」と改行で区切った要素 | 5,059 |
| 法令として読み取れた要素 | 3,380（うち条まで 3,206、別表 174） |
| 通達として読み取れた要素 | 1,387 |
| 読み取れなかった要素 | 292（5.8%） |
| 欄の全要素を読み取れた事例 | 1,652 / 1,834（90.1%） |
| `next_actions` の数（事例あたり） | 0 件が 373 事例、1〜3 件が 1,163 事例、10 件以上が 18 事例 |

### ローカル DB について

- `--bulk-download-qa` で取り込む本文（`full_text`）から注記が外れる。既存の DB に反映するには `houki-nta-mcp --bulk-download-qa --refresh` を実行する。全事例の `content_hash` が変わるため、health check が「一斉更新」を warn するが、解析結果が変わったためで国税庁側の異常ではない
- `nta_get_qa` は常に国税庁サイトから取得して解析するので、DB を取り直さなくても新しいフィールドは返る
- 検索（`nta_search_qa`）の結果には `related_laws` を付けていない。事例ごとの参照は `nta_get_qa` で取る
...
- `search_notes` の 0 件応答で、再検索の例文が固定（"役員退職" "役員給与"）だったのを、検索したキーワードに合わせた文（`"社宅" に語を続けて 3 文字以上にした形での再検索を推奨します`）に変更

### Changed

- `src/cli.test.ts` に `--refresh` → `forceReload` の経路のテスト 4 件を追加（`bulkDownloadTsutatsu` を mock、合計 **512 tests**）
