...
describe('取得系: その種別の文書が DB に 1 件も無いときは投入を案内する', () => {
  for (const [tool, , code, flag, , label, call] of cases) {
    it(`${tool}: code=${code}、next_actions に ${flag}、hint に DB のパス`, async () => {
...
describe('取得系: 文書がある DB で docId が無いときは「見つかりません」', () => {
  ...
  for (const [tool, docType, code, flag, searchTool, label, call] of cases) {
    it(`${tool}: 「見つかりません」、available_doc_ids、next_actions に ${searchTool}`, async () => {
    ...
    it(`${tool}: 別の種別の文書しか無い DB では投入を案内する`, async () => {
    ...
    it(`${tool}: DB にある docId はこれまでどおり取得できる`, async () => {
      const docId =
        docType === 'kaisei'
          ? '0025004-026'
          : docType === 'jimu-unei'
