  ...
  inputSchema: {
    ...
    properties: {
      ...
      clause: {
        type: 'string',
        description:
          '通達番号。形は通達ごとに違う。消費税法基本通達: 章-節-条（例 "5-1-9", "1-4-13の2"）。法人税基本通達: 章-節-条で、節に枝番号が付くことがある（例 "1-1-1", "1-3の2-1"）。所得税基本通達: 条-項。複数の条に共通する通達は "条~条共-項"（例 "34-1", "2-4の2", "23~35共-6"）。相続税法基本通達: 条-項。複数の条に共通する通達は "条・条共-項"（例 "3-1", "1の3・1の4共-1"）。DB から返すときは全角の数字・ハイフンでもよい。国税庁サイトから取るときに受け付けるのは 章-節-条 の形だけ',
      },
      format: {
        type: 'string',
        enum: [...OUTPUT_FORMATS],
