...
export function extractClauseNumber(
  ...
  // 3) 通常形式 (汎用): 各セグメントが「数字(のN)?」で構成され、ハイフンで連結
  //    対応:
  //      - "1-1-1" / "11-5-7"               (消基通 3 階層)
  //      - "1-4-13の2"                      (消基通 末尾の付き)
  //      - "1-3の2-1" / "1-3の2-2"          (法基通 第3節の2 配下 — 途中のセグメントも「のN」可能)
  //      - "23-1" / "2-4の2" / "3-1"        (所基通・相基通 2 階層)
  //      - "1の2-1" / "23の2-3"             (相基通 先頭セグメントが「のN」付き)
  //      - "1の2"                           (単一セグメント末尾「のN」のみは想定外、要なら追加)
